class ApiController < ApplicationController
  def random
   ...
   def clues
    ...
    clues = clues.where("id = ?", params[:id]) if params[:id].present?
    clues = clues.where('question LIKE ?', '%' + params[:keyword] + '%') if params[:keyword].present?
    ...
    clues = clues.where(category_id: JSON.parse(params[:category])) if params[:category].present?
  ...
  def rando_set
    category = Category.where("clues_count >= 5").order("RANDOM()").limit(5)
  ...
  def category_type  
    put "FUCK"
    Category.where("title LIKE ?", '%' + "states" + '%')
  end
